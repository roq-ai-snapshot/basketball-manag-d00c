...
import { getSportsTeamById, updateSportsTeamById } from 'apiSdk/sports-teams';
...
function SportsTeamEditPage() {
  ...
  const handleSubmit = async (values: SportsTeamInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSportsTeamById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<SportsTeamInterface>({
    initialValues: data,
    validationSchema: sportsTeamValidationSchema,
