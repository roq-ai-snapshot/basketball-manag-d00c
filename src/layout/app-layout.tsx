...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'team-manager', 'general-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SportsTeamInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/sports-teams` : null),
    () => getSportsTeams({ tenant_id: session?.user?.tenantId }),
  );
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Sports Team',
      path: isTenantUser ? `/sports-teams/view/${data?.[0]?.id}` : '/sports-teams',
      entity: 'sports_team',
